...
const actions = {
  ...
  doInit: (id) => async (dispatch) => {
    try {
      ...
      if (isEdit) {
        record = await AgentService.find(id);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      Message.success(
        i18n('entities.agent.create.success'),
      );
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await AgentService.update(id, values);
      ...
      Message.success(
        i18n('entities.agent.update.success'),
      );
