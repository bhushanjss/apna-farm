...
const actions = {
  ...
  doInit: (id) => async (dispatch) => {
    try {
      ...
      if (isEdit) {
        record = await SoilService.find(id);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      await SoilService.create(values);
      ...
      Message.success(
        i18n('entities.soil.create.success'),
      );
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await SoilService.update(id, values);
      ...
      Message.success(
        i18n('entities.soil.update.success'),
      );
