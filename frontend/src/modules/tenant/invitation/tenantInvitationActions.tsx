...
const actions = {
  ...
  doAcceptFromAuth: (
    ...
    try {
      ...
      dispatch({
        type: actions.ACCEPT_FROM_AUTH_STARTED,
      });
      ...
      dispatch({
        type: actions.ACCEPT_FROM_AUTH_SUCCESS,
      });
    } catch (error) {
      ...
      if (Errors.errorCode(error) === 400) {
        dispatch({
          type: actions.ACCEPT_FROM_AUTH_WARNING,
      ...
      Errors.handle(error);
      dispatch({
        type: actions.ACCEPT_FROM_AUTH_ERROR,
      });
      getHistory().push('/');
    }
  },
