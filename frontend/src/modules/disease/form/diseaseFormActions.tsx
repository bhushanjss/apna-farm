...
const actions = {
  ...
  doInit: (id) => async (dispatch) => {
    try {
      ...
      if (isEdit) {
        record = await DiseaseService.find(id);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      Message.success(
        i18n('entities.disease.create.success'),
      );
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await DiseaseService.update(id, values);
      ...
      Message.success(
        i18n('entities.disease.update.success'),
      );
