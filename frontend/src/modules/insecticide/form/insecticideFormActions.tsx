...
const actions = {
  ...
  doInit: (id) => async (dispatch) => {
    try {
      ...
      if (isEdit) {
        record = await InsecticideService.find(id);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      await InsecticideService.create(values);
      ...
      Message.success(
        i18n('entities.insecticide.create.success'),
      );
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await InsecticideService.update(id, values);
      ...
      Message.success(
        i18n('entities.insecticide.update.success'),
      );
