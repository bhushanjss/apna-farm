...
const actions = {
  ...
  doInit: (id) => async (dispatch) => {
    try {
      ...
      if (isEdit) {
        record = await PesticideService.find(id);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      await PesticideService.create(values);
      ...
      Message.success(
        i18n('entities.pesticide.create.success'),
      );
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await PesticideService.update(id, values);
      ...
      Message.success(
        i18n('entities.pesticide.update.success'),
      );
