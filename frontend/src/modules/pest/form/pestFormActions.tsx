...
const actions = {
  ...
  doInit: (id) => async (dispatch) => {
    try {
      ...
      if (isEdit) {
        record = await PestService.find(id);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      await PestService.create(values);
      ...
      Message.success(
        i18n('entities.pest.create.success'),
      );
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await PestService.update(id, values);
      ...
      Message.success(
        i18n('entities.pest.update.success'),
      );
