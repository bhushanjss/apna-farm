...
const actions = {
  ...
  doInit: (id) => async (dispatch) => {
    try {
      ...
      if (isEdit) {
        record = await FertilizerService.find(id);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      Message.success(
        i18n('entities.fertilizer.create.success'),
      );
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await FertilizerService.update(id, values);
      ...
      Message.success(
        i18n('entities.fertilizer.update.success'),
      );
